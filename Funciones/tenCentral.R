tablas_mtc <- function(tabla) {
  ...
  # Media, armónica, geométrica, cuadrática (RMS)
  mu  <- sum(f * mc) / N
  H   <- N / sum(f / mc)
  G   <- 10^(sum(f * log10(mc)) / N)
  RMS <- sqrt(sum(f * mc^2) / N)
  
  # Mediana (cuantil p = 0.5)
  ...
  data.frame(
    Medida = c("Media","Media Armonica","Media Geometrica","Media Cuadratica","Mediana","Moda"),
